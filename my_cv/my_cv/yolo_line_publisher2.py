...
class YoloNode(Node):
    def __init__(self):
        ...
        with open("/home/rnd/my_cv/best_fp16.engine", "rb") as f, trt.Runtime(TRT_LOGGER) as runtime:
            self.engine = runtime.deserialize_cuda_engine(f.read())
        self.execution_context = self.engine.create_execution_context()

        # I/O 버퍼 준비
        self.input_shape = (3, 416, 416)  # YOLOv5 고정 입력
        self.input_size = np.prod(self.input_shape).item()
        self.output_size = 25200 * 9  # YOLOv5s 기준 (classes + 4 + objectness)

        self.input_host = cuda.pagelocked_empty((self.input_size,), dtype=np.float32)
        self.output_host = cuda.pagelocked_empty((self.output_size,), dtype=np.float32)
    ...
    def image_callback(self, msg):
        ...
        image_resized = cv2.resize(rgb_frame, (416, 416	)).astype(np.float32) / 255.0
        image_chw = np.transpose(image_resized, (2, 0, 1)).flatten()
        np.copyto(self.input_host, image_chw)	
        
        # stream 만들기
        stream = cuda.Stream()
        
        self.execution_context.set_input_shape("images", (1, 3, 416, 416))

        # 반드시 먼저 입력/출력 주소 등록
        self.execution_context.set_tensor_address("images", int(self.input_device))
        self.execution_context.set_tensor_address("output0", int(self.output_device))

        # 복사 & 추론 실행
        cuda.memcpy_htod_async(self.input_device, self.input_host, stream)
        self.execution_context.execute_async_v3(stream_handle=stream.handle)
        cuda.memcpy_dtoh_async(self.output_host, self.output_device, stream)

        stream.synchronize()

        # 결과 처리
        output = self.output_host.reshape((25200, 9))  # YOLOv5s 기준
        ...
        centers = []
        for box in boxes:
            x1, y1, x2, y2, obj_conf, class_conf, class_id = box[:7]
            if int(class_id) == 0:  # 'line' 클래스 인덱스가 0번이라고 가정
                ...
                x1_int, y1_int, x2_int, y2_int = int(x1 * screen_w), int(y1 * screen_h), int(x2 * screen_w), int(y2 * screen_h)
                cv2.rectangle(bgr_frame, (x1_int, y1_int), (x2_int, y2_int), (0, 255, 0), 2)
                cv2.circle(bgr_frame, (cx, cy), 3, (0, 0, 255), -1)
                cv2.putText(bgr_frame, f"line {obj_conf:.2f}", (x1_int, y1_int - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
        ...
        cv2.waitKey(1)
        print(self.last_avg_text)
        print(output[:5])


    def postprocess(self, output, conf_thresh=1e-5):
        boxes = output[:, :4]
        obj_conf = output[:, 4]
        cls_confs = output[:, 5:]
        class_ids = np.argmax(cls_confs, axis=1)
        class_conf = cls_confs[np.arange(len(cls_confs)), class_ids]
        scores = obj_conf * class_conf
        
        keep = scores > conf_thresh
        final_boxes = np.concatenate([boxes[keep], obj_conf[keep, None], class_conf[keep, None], class_ids[keep, None]], axis=1)

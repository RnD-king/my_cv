...
#---------------------------------------------------------------1차 판단 시작  << 최소자승 직선 근사  << 얘 좀 이상함
        ...
        if len(self.candidates) >= 3:  # 1. 3개 이상일 때: 맨 위 점 제외하고 직선 근사        
            ...
            if tip_lost > 0:
                cv2.circle(roi, (tip_x - roi_x_start, tip_y - roi_y_start), 3, (0, 255, 255), 2) # lost가  0이상이면 노랑
            else:
                cv2.circle(roi, (tip_x - roi_x_start, tip_y - roi_y_start), 4, (0, 0, 255), -1) # 가장 위의 새로운 점은 빨강
            
            for acx, acy, ac_lost in self.candidates[:-1]:
                if ac_lost > 0:
                    cv2.circle(roi, (acx - roi_x_start, acy - roi_y_start), 3, (0, 255, 255), 2) # lost가  0이상이면 노랑
                else:
                    cv2.circle(roi, (acx - roi_x_start, acy - roi_y_start), 4, (255, 0, 0), -1) # 나머지 파랑
            ...
            cv2.line(roi, (x1 - roi_x_start, y1 - roi_y_start), (x2 - roi_x_start, y2 - roi_y_start), (255, 0, 0), 2)
            ...
            if down_lost > 0:
                cv2.circle(roi, (x1 - roi_x_start, y1 - roi_y_start), 3, (0, 255, 255), 2) # lost가  0이상이면 노랑
            else:
                cv2.circle(roi, (x1 - roi_x_start, y1 - roi_y_start), 4, (0, 0, 255), -1) #  파랑
            if up_lost > 0:
                cv2.circle(roi, (x2 - roi_x_start, y2 - roi_y_start), 3, (0, 255, 255), 2) # lost가  0이상이면 노랑
            else:
                cv2.circle(roi, (x2 - roi_x_start, y2 - roi_y_start), 4, (255, 0, 0), -1) # 빨강

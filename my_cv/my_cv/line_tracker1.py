...
class ImgSubscriber(Node):
    ...
    def color_image_callback(self, msg):
                ...
                if len(approx) == 4:  # 1. 꼭짓점이 4개면,
                    ...
                    if 3000 > area_rect > 500:  # 2. 외접 박스 면적이 500 이상, 3000이하면,
                        ...
                        if all(60 <= angle <= 120 for angle in angles): # 3. 내각이 60~120 사이면,
                            cv2.drawContours(rgb_image, [approx], 0, (200, 0, 255), 2)  # 테두리 핑크 / 사각형 검출 하는지 확인용 / 지금 프레임에서 사각형이라고 검출된 모든 사각형
            ...
            for id,(cx,cy,lost,found) in valid_rects.items(): 
                cv2.putText(rgb_image, f"ID: {id}", (cx, cy - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2) # 유효 사각형은 id 출력
            ...
            stable_curve = Counter(self.recent_curve).most_common(1)[0][0] if self.recent_curve else "Miss"  # 최빈값에 맞게 커브 판단

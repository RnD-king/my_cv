...
class LineListenerNode(Node): #################################################################### 판단 프레임 수 바꿀 때 yolo_cpp 도 고려해라~~~
    def __init__(self):
        ...
        self.declare_parameter("orange_h_high", 15)  
        self.declare_parameter("orange_s_low", 150) # 채도  40, 255
        self.declare_parameter("orange_s_high", 255)  
        self.declare_parameter("orange_v_low", 170) # 밝기 50, 255
        self.declare_parameter("orange_v_high", 255)

        self.declare_parameter("orange2_h_low", 1) # 주황 8, 20 < 동방 기준임
        self.declare_parameter("orange2_h_high", 30)  
        self.declare_parameter("orange2_s_low", 100) # 채도  40, 255
        self.declare_parameter("orange2_s_high", 255)  
        self.declare_parameter("orange2_v_low", 150) # 밝기 50, 255
    ...
    def apply_mode_layout(self):
        if self.cam_mode == CAM1:
            if self.cam1_mode == BALL:
                ...
                self.roi_x_start, self.roi_x_end = 0, camera_width
                self.roi_y_start, self.roi_y_end = camera_height*4//12, camera_height*10//12
        else:  # CAM2
            self.roi_x_start, self.roi_x_end = 0, camera_width

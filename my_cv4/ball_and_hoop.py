...
class LineListenerNode(Node): #################################################################### 판단 프레임 수 바꿀 때 yolo_cpp 도 고려해라~~~
    def __init__(self):
        ...
        self.declare_parameter("orange_h_low", 8) # 주황 8, 60 < 동방 기준임
        self.declare_parameter("orange_h_high", 60)  
        self.declare_parameter("orange_s_low", 60) # 채도  40, 255
    ...
    def apply_mode_layout(self):
        if self.cam_mode == CAM1:
            if self.cam1_mode == BALL:
                ...
                self.roi_x_start, self.roi_x_end = 0, camera_width
                self.roi_y_start, self.roi_y_end = camera_height*4//12, camera_height*12//12
        else:  # CAM2
            self.roi_x_start, self.roi_x_end = 0, camera_width
    ...
    def cam1_image_callback(self, cam1_color_msg: Image, cam1_depth_msg: Image): # yolo_cpp에서 토픽 보내면 실핼
        ...
        if self.cam1_mode == BALL: ##################################################################################################3
            ...
            if self.collecting:
                ...
                if self.frames_left <= 0:
                    ...
                    if result == True: # 공 찾았을 때
                        ...
                        if self.cam1_ball_saw_once:
                            self.cam1_miss_count += 1
                            if self.last_avg_cy_ball >= 300: # 그 전까지 공을 보고 있었고 공이 대충 밑에 있었다면
                                ...
                                if self.cam1_miss_count <= 2:  # 찾았었는데 놓쳤다면
                                ...
                                elif self.cam1_miss_count <= 4:  # 찾았었는데 놓쳤다면
                                    self.get_logger().info(f"[Ball] finding one more")
                                    res = 12 # 제자리에서
                                    angle = 0
            ...
            cv2.line(frame, (self.roi_x_start, 300), (self.roi_x_end, 300), (250, 122, 122), 2)
                ...
                if self.frames_left <= 0:
                    ...
                    if result == True: # 탐지가 더 많음
                        ...
                        if self.hoop_near_by:
                            ...
                            elif self.hoop_miss_count <= 3: # 근접했는데 놓쳤으면
                                res = 97 # 다시 찾는 모션 >> 제자리에서
                                angle = 0

                                self.get_logger().info(f"[Hoop] Try again,,, | miss= {self.hoop_miss_count}")
                                self.last_position_text = "No hoop"
                                
                            elif self.hoop_miss_count <= 5: # 근접했는데 놓쳤으면 
                                res = 5 # 다시 찾는 모션 >> 한발짝 뒤에서
                                angle = 0

                                self.get_logger().info(f"[Hoop] Try again,,, | miss= {self.hoop_miss_count}")
                                self.last_position_text = "No hoop"

                            else: # GG요 (5번 이상 놓쳤을 때) <<<<<<<<<<<<<<<<<<<<<<<<

...
class LineListenerNode(Node): #################################################################### 판단 프레임 수 바꿀 때 yolo_cpp 도 고려해라~~~
    def __init__(self):
        ...
        self.declare_parameter("orange_h_low", 8) # 주황 8, 20 < 동방 기준임
        self.declare_parameter("orange_h_high", 20)  
        self.declare_parameter("orange_s_low", 150) # 채도  40, 255
        self.declare_parameter("orange_s_high", 255)  
        self.declare_parameter("orange_v_low", 130) # 밝기 50, 255
        self.declare_parameter("orange_v_high", 255)

        self.declare_parameter("orange2_h_low", 1) # 주황 8, 20 < 동방 기준임
        self.declare_parameter("orange2_h_high", 45)  
        self.declare_parameter("orange2_s_low", 20) # 채도  40, 255
        self.declare_parameter("orange2_s_high", 255)  
        self.declare_parameter("orange2_v_low", 100) # 밝기 50, 255
        self.declare_parameter("orange2_v_high", 255)
        ...
        self.orange2_h_low = self.get_parameter("orange2_h_low").value
        self.orange2_h_high = self.get_parameter("orange2_h_high").value
        self.orange2_s_low = self.get_parameter("orange2_s_low").value
        self.orange2_s_high = self.get_parameter("orange2_s_high").value
        self.orange2_v_low = self.get_parameter("orange2_v_low").value
        self.orange2_v_high = self.get_parameter("orange2_v_high").value
        ...
        self.lower_hsv_ball2 = np.array([self.orange2_h_low, self.orange2_s_low, self.orange2_v_low], dtype=np.uint8)  # 두 번째 필터용
        self.upper_hsv_ball2 = np.array([self.orange2_h_high, self.orange2_s_high, self.orange2_v_high], dtype=np.uint8)  # 두 번째 필터용
        self.hsv = None 
    ...
    def param_callback(self, params):
        ...
        for p in params:
            ...
            elif p.name == "orange2_h_low":   self.orange2_h_low = int(p.value)
            elif p.name == "orange2_h_high": self.orange2_h_high = int(p.value)
            elif p.name == "orange2_s_low":  self.orange2_s_low = int(p.value)
            elif p.name == "orange2_s_high": self.orange2_s_high = int(p.value)
            elif p.name == "orange2_v_low":  self.orange2_v_low = int(p.value)
            elif p.name == "orange2_v_high": self.orange2_v_high = int(p.value)
    ...
    def cam1_image_callback(self, cam1_color_msg: Image, cam1_depth_msg: Image): # yolo_cpp에서 토픽 보내면 실핼
        ...
        if self.cam1_mode == BALL: ##################################################################################################3
            ...
            if self.collecting:
                ...
                contours_large = [cnt for cnt in contours if cv2.contourArea(cnt) >= 400]
            ...
            if self.collecting:
                ...
                # 모폴로지
                # (A) 색/깊이 마스크까지 끝난 직후 — 모폴로지 하기 전에 0차 면적 컷
                red_mask_raw = red_mask.copy()

                min_piece_area0 = getattr(self, 'min_piece_area0', 300)  # px 단위(튜닝)
                # backboard_area 비례로 쓰고 싶으면 예: int(0.02 * self.backboard_area)
                ...
                for c in cnts0:
                    if cv2.contourArea(c) >= min_piece_area0:
                        cv2.drawContours(red_mask_filtered0, [c], -1, 255, thickness=cv2.FILLED)

                # 이제 모폴로지를 red_mask_filtered0에 적용
                red_mask = cv2.morphologyEx(red_mask_filtered0, cv2.MORPH_OPEN,  self.kernel)
                ...
                # 디버그 확인용
                if getattr(self, 'debug_masks', False):
                    cv2.imshow('DBG/red_mask_raw', red_mask_raw)
                    cv2.imshow('DBG/red_mask_filtered0', red_mask_filtered0)
                    cv2.imshow('DBG/red_mask_after_morph', red_mask)
                    ...
                    if area > self.backboard_area: # 1. 일정 넓이 이상
                        ...
                        if ratio_top >= self.red_ratio_min and ratio_left >= self.red_ratio_min and ratio_right >= self.red_ratio_min:
                            ...
                            if best_score < ratio_band:
                                ...
                                # 시각화용 박스는 "shrunk-rect" 추천 (원 rect는 팽창/침식前 기준이라 혼동됨)
                ...
                if self.frames_left <= 0:
                    ...
                    if result == True: # 탐지가 더 많음
                        ...
                        if 1200 < avg_dis: # 거리가 1.2m 이상 > 일단 골대 쪽으로 직진하기
                            ...
                            z_ = avg_dis * math.cos(beta_rad) - self.hoop_r * math.cos(avg_yaw_rad) - self.throwing_range # x_, z_ 내 기준 링 중심 좌표

                            if math.hypot(x_,z_) < self.hoop_r:
                                res = 17
                                self.last_score = self.last_top_score = self.last_left_score = self.last_right_score = None
                                self.last_cx_hoop = self.last_cy_hoop = self.last_z_hoop = self.last_yaw = None
                                self.last_box_hoop = None
